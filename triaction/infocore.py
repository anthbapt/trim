...
def timeseries_quantile(timeseries:np.ndarray, I:list, num:int, tlen:int):
    ...
    X_sort : numpy.ndarray
        Sorted values of the first row of interest.
    Y_sort : numpy.ndarray
        Sorted values of the second row of interest.
    sp : numpy.ndarray
        Quantile labels corresponding to the third row of interest.
    """
...
def mutual_information_analysis_continuous(timeseries:np.ndarray, I:list, num:int, tlen:int):
    ...
    tlen : int
        Length of the time window considered for analysis.

    Returns
    ----------
    MI : float 
        Mutual information between the first and second rows of interest.
    MIz : numpy.ndarray
        Mutual information values for each interval.
    MIC : float
        Mean mutual information across intervals.
    Sigma : float
        Standard deviation of mutual information values.
    T : float
        Range of mutual information values across intervals.
    Tn : float
        Maximum difference between consecutive mutual information values.
    """
...
def mutual_information_analysis_continuous_extended(timeseries:np.ndarray, I:list, num:int, tlen:int):
    ...
    MIC : float
        Mean mutual information across intervals.
    Corr_Sigma : float
        Pearson correlation between mean Zn values and MIz.
    Sigma : float
        Standard deviation of mutual information values.
    T : float
        Range of mutual information values across intervals.
    Tn : float
        Maximum difference between consecutive mutual information values.
    MINDY : float
        Absolute difference between mutual information values of the first and last third of the data.
    MI1 : float
        Mutual information of the first third of the data.
    MI2 : float
        Mutual information of the last third of the data.
    """
...
def mindy_final(timeseries:np.ndarray, I:list, tlen:int):
    ...
    ----------
    MINDY : float
        Mutually Information Normalized Difference Yield.
    MI1 : float
        Mutual information of the first third of the data.
    MI2 : float
        Mutual information of the last third of the data.
...
def sigma(timeseries:np.ndarray, I:list, num:int, tlen:int, null:bool = True, nrunmax:int = 5000):
    ...
    tlen : int
        Length of the time window considered for analysis.
    null : bool or int
        If False, only computes the standard deviation. If an integer, simulates a null model 
        with the specified number of runs. Defaults to False.
    nrunmax : int
        Maximum number of runs for null model simulation. Default is 5000.

    Returns
    ----------
    Sigma : float
        Standard deviation of conditional mutual information
    Sigma_null : 
        Standard deviation of conditional mutual information of null model         
    """
...
def t(timeseries:np.ndarray, I:list, num:int, tlen:int, null:bool = True, nrunmax:int = 5000):
    ...
    ----------
    T : float
        Absolute difference between min and max values of conditional mutual information
    T_null : 
        Absolute difference between min and max values of conditional mutual information of null model  
    """
...
def tn(timeseries:np.ndarray, I:list, num:int, tlen:int, null:bool = True, nrunmax:int = 5000):
    ...
    ----------
    Tn : float
        Absolute difference between two consecutive conditional mutual information value
    Tn_null : 
        Absolute difference between two consecutive conditional mutual information value of null model 
    """
...
def correlation_analysis_continuous(timeseries:np.ndarray, I:list, num:int, tlen:int):
    ...
    ----------
    C : float
        Mean correlation between the first and second rows of interest.
    Cz : numpy.ndarray
        Correlation values for each interval.
    Sigma : float
        Variance of correlation values.
    T : float
        Range of correlation values across intervals.
    Tn : float
        Maximum difference between consecutive correlation values.
    """
...
def null_model_results(M:np.ndarray, Cov:np.ndarray, timeseries:np.ndarray, I:list, num:int, tlen:int, Sigma:float, T:float, Tn:float, nrunmax:int, Gaussian_version:bool, Mutual_version:bool, model:int = None):
    """
    ...
    tlen : int
        Length of the time window considered for analysis.
    Sigma : float
        Standard deviation of mutual information
    T : float
        Range of mutual information values in the original data.
    Tn : float
        Maximum difference between consecutive mutual information values in the original data.
    nrunmax : int
        Maximum number of runs for null model simulation.
    Gaussian_version : bool
        If True, uses a multivariate normal distribution as a null model.
    Mutual_version : bool
        If True, performs mutual information analysis; otherwise, performs correlation analysis.
    model : int
        Null model type (0-7). Defaults to None.

    Returns
    ----------
    X_null : float
        Mean value of mutual information or correlation in null model.
    Xz_null : numpy.ndarray
        Values of mutual information or correlation for each interval in null model.
    Theta : float
        Normalized difference in standard deviation of the original and null models.
    Theta_T : float
        Normalized difference in range of the original and null models.
    Theta_Tn : float
        Normalized difference in maximum difference of the original and null models.
    Sigma : float
        Standard deviation of mutual information values in the original data.
    Sigma_null_list : numpy.ndarray
        Standard deviation of mutual information values for each run in null model.
    P : float
        Probability of observing a standard deviation greater than the original in the null model.
    P_T : float
        Probability of observing a range greater than the original in the null model.
    P_Tn : float
        Probability of observing a maximum difference greater than the original in the null model.
...
def null_model(timeseries:np.ndarray, I:list, num:int, tlen:int, nrunmax:int, Gaussian_version:bool, Mutual_version:bool, model:int = None):
    """
    ...
    Returns
    ----------
    Sigma_mean_null : float
        Mean standard deviation of mutual information values in null model.
    T_mean_null : float
        Mean range of mutual information values in null model.
    Tn_mean_null : float
        Mean maximum difference between consecutive mutual information values in null model.
...
def Theta_score_null_model(timeseries:np.ndarray, I:list, num:int, tlen:int, nrunmax:int, Gaussian_version:bool = True, Mutual_version:bool = True, extended:bool = False):
    """
    ...
    Xz : numpy.ndarray
        Sorted input data for analysis.
    Xz_null : numpy.ndarray
        Sorted null model input data.
    MIC : float
        Mutual information or correlation value for the original data.
    Theta : float
        Theta score, a measure of the difference between the original data and the null model.
    Theta_T : float
        Theta score for the range of the original data.
    Theta_Tn : float
        Theta score for the maximum difference between consecutive values in the original data.
    Sigma : float
        Standard deviation of mutual information values for the original data.
    Sigma_null_list : numpy.ndarray
        List of standard deviations of mutual information values from null model simulations.
    P : float
        P-value indicating the statistical significance of Theta score.
    P_T : float
        P-value indicating the statistical significance of Theta_T score.
    P_Tn : float
        P-value indicating the statistical significance of Theta_Tn score.
    """
...
def freedman_diaconis(data:np.ndarray, returnas:str = "bins"):
    ...
    ----------
    data : numpy.ndarray
        Input data for which the bin width is computed.
    returnas : str
        If "bins", returns the number of bins; if "width", returns the bin width. Defaults to "bins".
...
def mch_approximation(samples:np.ndarray, dlamda:np.ndarray):
    ...
    samples : numpy.ndarray
        Array of Ising model samples.
    dlamda : numpy.ndarray
        Array of changes in parameters for the Ising model.

    Returns
    ----------
    predsisj : numpy.ndarray
        Predicted values for the mean Ising model observables.
