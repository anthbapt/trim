...
#        g0 = sns.jointplot(x = X_a, y = Y_a, label = 'Z < '+f"z$_{{{cond[0]}}}$", xlim = (xmin,xmax), ylim = (ymin,ymax))
        ...
        plt.legend(fontsize='15')
#        g1 = sns.jointplot(x = X_b, y = Y_b, label = f"z$_{{{cond[0]}}}$" + '≤ Z ≤ '+f"z$_{{{cond[1]}}}$", xlim = (xmin,xmax), ylim = (ymin,ymax))
        ...
        plt.legend(fontsize='15')
#        g2 = sns.jointplot(x = X_c, y = Y_c, label = f"z$_{{{cond[1]}}}$" + '< Z', xlim = (xmin,xmax), ylim = (ymin,ymax))
...
def visualisation_conditioned_val(timeseries, I, num, tlen, name:str = None, cond = None):
    """
    Visualize conditioned distributions.
    ...
    if cond == None:
        ...
        X_inf = X[Z < cond_val]
        Y_inf = Y[Z < cond_val]
        val = round(np.quantile(Z_sort,cond_val/num),4)
        xmin, xmax = 0, np.max(np.concatenate((X, Y)))
        ymin, ymax = 0, np.max(np.concatenate((X, Y)))
    ...
    if type(cond) is list:
        ...
        X_c = X[Z > cond[1]]
        Y_c = Y[Z > cond[1]]
        val1 = round(np.quantile(Z_sort,cond[0]/num),4)
        val2 = round(np.quantile(Z_sort,cond[1]/num),4)
        print('val',val1,val2)
        xmin, xmax = np.min(np.concatenate((X, Y))), np.max(np.concatenate((X, Y)))
        ymin, ymax = np.min(np.concatenate((X, Y))), np.max(np.concatenate((X, Y)))
...
def decision_tree(x_1d, y_1d, disp_fig = False, disp_txt_rep = False, disp_tree = False, name = None):
    ...
    if disp_fig == True or name is not None:
        ...
        plt.xlabel("$\mathregular{m_z}$", fontsize = 20)    
        plt.ylabel("${MI_z}$", fontsize = 20)
...
def decision_tree_val(x_1d, y_1d, z, disp_fig = False, disp_txt_rep = False, disp_tree = False, name = None):
    """
    Create and visualize a Decision Tree model for predicting mutual information.
    ...
    idx = z.argsort()
    z_sort = z[idx]
    z_val = np.round(np.array([np.quantile(z_sort,i) for i in np.arange(1,len(y_1d)+1)/len(y_1d)]),4)
    size = int(np.max(x_1d))
    x = np.zeros((size,1))
    y = np.zeros((size,1))
    x[0:size,0] = x_1d
...
#        plt.title('Decision tree', fontsize = 15)
        ...
        plt.ylabel("MI(XY|Z = z)", fontsize = 20)
        plt.xticks(ticks=x_1d[::66], labels=z_val[::66])
