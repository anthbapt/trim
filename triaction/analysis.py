...
def visualisation_conditioned(timeseries:np.ndarray, I:list, num:int, tlen:int, name:str = None, cond:float = None):
    ...
    if type(cond) is list:
        ...
        xmin, xmax = np.min(np.concatenate((X, Y))), np.max(np.concatenate((X, Y)))
        ymin, ymax = np.min(np.concatenate((X, Y))), np.max(np.concatenate((X, Y)))
        g0 = sns.jointplot(x = X_a, y = Y_a, label = 'Z < '+str(cond[0]), xlim = (-1,xmax), ylim = (-1,ymax))
        g0.ax_joint.set_xlabel('X', fontsize=15)
        g0.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='15')
        g1 = sns.jointplot(x = X_b, y = Y_b, label = str(cond[0]) + '<= Z <= '+str(cond[1]), xlim = (-1,xmax), ylim = (-1,ymax))
        g1.ax_joint.set_xlabel('X', fontsize=15)
        g1.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='15')
        g2 = sns.jointplot(x = X_c, y = Y_c, label = str(cond[1]) + '< Z', xlim = (-1,xmax), ylim = (-1,ymax))
