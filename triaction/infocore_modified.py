...
def timeseries_quantile_val(timeseries, I, num, tlen):
    """
    Computes quantiles of time series data.
...
def mindy_final(timeseries, I, tlen):
    """
...
def sigma_final(timeseries, I, num, tlen, null = True, nrunmax = 5000):
    """
...
def sigma(timeseries, I, num, tlen, null = True, nrunmax = 5000):
    """
    Computes the standard deviation of mutual information values between continuous variables in a time series.
    Define as the Sigma measure.
    ...
    for i in range(num):
        ...
        mi = mutual_info_regression(Xn, y, discrete_features = False)
        MIz[i] = mi[0]
        oldMIz[i] = drv.information_mutual(np.round(Xn[:,0]), np.round(y))
        
    oldSigma = np.std(oldMIz)
    Sigma = np.std(MIz)
    if null == False:
        return Sigma, oldSigma
    else:
        if type(null) == int:
            Sigma_null, T_null, Tn_null = null_model(timeseries, I, num, tlen, nrunmax, Gaussian_version = True,\
                                                                    Mutual_version = True, model = None)
            
            return Sigma, Sigma_null, oldSigma
    
        else:
            raise TypeError("Only integers or False accepted")
            

def t(timeseries, I, num, tlen, null = True, nrunmax = 5000):
    """
...
def tn(timeseries, I, num, tlen, null = True, nrunmax = 5000):
    """
