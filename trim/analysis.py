...
def visualisation_conditioned(timeseries, I, num, tlen, name: str = None, cond = None):
    ...
    if cond is None:
        ...
        g0 = sns.jointplot(x = X_sup, y = Y_sup, label = 'Z > ' + rf"$z_{cond_val}$", xlim = (xmin,xmax), ylim = (ymin,ymax))
        g0.ax_joint.set_xlabel('X', fontsize=15)
        g0.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='15')
        g1 = sns.jointplot(x = X_inf, y = Y_inf, label = 'Z < ' + rf"$z_{cond_val}$", xlim = (xmin,xmax), ylim = (ymin,ymax))
    ...
    if isinstance(cond, int):
        ...
        xmin, xmax = 0, np.max(np.concatenate((X, Y)))
        ymin, ymax = 0, np.max(np.concatenate((X, Y)))
        g0 = sns.jointplot(x = X_inf, y = Y_inf, label = 'Z < ' + rf"$z_{cond_val}$", xlim = (xmin,xmax), ylim = (ymin,ymax))
        g0.ax_joint.set_xlabel('X', fontsize=15)
        g0.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='15')
        g1 = sns.jointplot(x = X_sup, y = Y_sup, label = 'Z > ' + rf"$z_{cond_val}$", xlim = (xmin,xmax), ylim = (ymin,ymax))
        g1.ax_joint.set_xlabel('X', fontsize=15)
        g1.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='15')
    ...
    if isinstance(cond, list):
        ...
        plt.legend(fontsize='15')
        g1 = sns.jointplot(x = X_b, y = Y_b, label = str(round(max(Z_sort[Z < cond[0]]),2)) + r'$\mathregular{\leq}$' + ' Z ' +  r'$\mathregular{\leq}$' + str(round(min(Z_sort[Z > cond[1]]),2)), xlim = (xmin,xmax), ylim = (ymin,ymax))
...
def visualisation_conditioned_val(timeseries: np.ndarray, I: list, num: int, tlen: int, name: str = None, cond: float = None):
    ...
    if cond is None:
        ...
        xmin, xmax = 0, np.max(np.concatenate((X, Y)))
        ymin, ymax = 0, np.max(np.concatenate((X, Y)))
        g0 = sns.jointplot(x = X_sup, y = Y_sup, label = r'$Z > {}$'.format(val), xlim = (xmin,xmax), ylim = (ymin,ymax))
        g0.ax_joint.set_xlabel('X', fontsize=15)
        g0.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='16')
        g1 = sns.jointplot(x = X_inf, y = Y_inf, label = r'$Z < {}$'.format(val), xlim = (xmin,xmax), ylim = (ymin,ymax))
        g1.ax_joint.set_xlabel('X', fontsize=15)
        g1.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='16')
    ...
    if isinstance(cond, int):
        ...
        xmin, xmax = 0, np.max(np.concatenate((X, Y)))
        ymin, ymax = 0, np.max(np.concatenate((X, Y)))
        g0 = sns.jointplot(x = X_inf, y = Y_inf, label = 'Z < {}'.format(val), xlim = (xmin,xmax), ylim = (ymin,ymax))
        g0.ax_joint.set_xlabel('X', fontsize=15)
        g0.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='12')
        g1 = sns.jointplot(x = X_sup, y = Y_sup, label = 'Z > {}'.format(val), xlim = (xmin,xmax), ylim = (ymin,ymax))
        g1.ax_joint.set_xlabel('X', fontsize=15)
        g1.ax_joint.set_ylabel('Y', fontsize=15)
        plt.legend(fontsize='12')
    ...
    if isinstance(cond, list):
        ...
        g0 = sns.jointplot(x = X_a, y = Y_a, label = 'Z < '+ r'$\mathregular{z_{i}}$'.replace('i',str(cond[0])), xlim = (xmin,xmax), ylim = (ymin,ymax))
        g0.ax_joint.set_xlabel('X', fontsize=18)
        g0.ax_joint.set_ylabel('Y', fontsize=18)
        plt.legend(fontsize='16')
        g1 = sns.jointplot(x = X_b, y = Y_b, label =r'$\mathregular{{z_{}}}$'.format(cond[0]) + r'$\mathregular{\leq}$' + 'Z' + r'$\mathregular{\leq}$' + r'$\mathregular{{z_{}}}$'.format(cond[1]), xlim = (xmin,xmax), ylim = (ymin,ymax))
        g1.ax_joint.set_xlabel('X', fontsize=18)
        g1.ax_joint.set_ylabel('Y', fontsize=18)
        plt.legend(fontsize='16')
        g2 = sns.jointplot(x = X_c, y = Y_c, label = r'$\mathregular{z_{i}}$'.replace('i',str(cond[1])) + '< Z', xlim = (xmin,xmax), ylim = (ymin,ymax))
...
def decision_tree(x_1d: np.ndarray, y_1d: np.ndarray, disp_fig: bool = False, disp_txt_rep: bool = False, disp_tree: bool = False, name: str = None):
    ...
    if disp_fig == True or name is not None:
        ...
        plt.xlabel(r"$\mathregular{z^{th}}$ quantile", fontsize = 12)
